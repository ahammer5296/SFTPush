...
class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate { // Изменено на UNUserNotificationCenterDelegate
    ...
    func userNotificationCenter(_ center: UNUserNotificationCenter, didReceive response: UNNotificationResponse, withCompletionHandler completionHandler: @escaping () -> Void) {
        ...
        if response.actionIdentifier == UNNotificationDefaultActionIdentifier,
           ...
           let url = URL(string: urlString) {
            ...
            print("Открываем URL из уведомления: \(urlString)")
        }
        completionHandler()
    }
    ...
    @objc func handleUploadSuccess(notification: Notification) {
        ...
        if let userInfo = notification.userInfo,
           ...
           let url = userInfo["url"] as? String {
            // Передаем URL в userInfo уведомления для клика по уведомлению
            sendNotification(title: "Загрузка завершена", subtitle: fileName, body: "Нажмите для перехода по ссылке", userInfo: ["url": url])
    ...
    func sendNotification(title: String, subtitle: String?, body: String, userInfo: [AnyHashable: Any]? = nil) {
        let defaults = UserDefaults.standard
    ...
    @objc func handleBatchUploadFinished(notification: Notification) {
        if let summary = notification.userInfo,
           ...
           let error = summary["error"] as? Int {
            ...
            // Добавим обновление для остановки анимации загрузки после массовой загрузки
