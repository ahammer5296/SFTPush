...
class SettingsViewController: NSViewController {
    ...
    let closeMonosnapAfterUploadCheckbox = NSButton(checkboxWithTitle: "Закрывать окно Monosnap после загрузки", target: nil, action: nil)
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            tabView.bottomAnchor.constraint(equalTo: view.bottomAnchor, constant: -20) // Нормальное размещение
        ])
    ...
    private func setupGeneralTab(in view: NSView) {
        ...
        closeMonosnapAfterUploadCheckbox.target = self
        closeMonosnapAfterUploadCheckbox.action = #selector(closeMonosnapAfterUploadChanged)
    ...
    override func loadView() {
        self.view = NSView(frame: NSRect(x: 0, y: 0, width: 480, height: 380)) // Нормальный размер окна настроек
    }
    ...
    private func setupBufferAndHotkeysTab(in view: NSView) {
        // Создаем вертикальный Stack View для компактного размещения чекбоксов
        ...
        checkboxStackView.spacing = 2 // Минимальный отступ между чекбоксами

        // Создаем заголовок для блока чекбоксов
        let copySettingsHeader = NSTextField(labelWithString: "Настройки копирования")
        copySettingsHeader.translatesAutoresizingMaskIntoConstraints = false
        copySettingsHeader.font = NSFont.boldSystemFont(ofSize: NSFont.systemFontSize)

        // Добавляем заголовок и чекбоксы в Stack View
        checkboxStackView.addArrangedSubview(copySettingsHeader)
        checkboxStackView.addArrangedSubview(copyBeforeUploadCheckbox)
        checkboxStackView.addArrangedSubview(copyOnlyFromMonosnapCheckbox)
        checkboxStackView.addArrangedSubview(closeMonosnapAfterUploadCheckbox)
        checkboxStackView.addArrangedSubview(uploadCopiedFilesCheckbox)

        // Настраиваем отступы для визуальной иерархии
        copySettingsHeader.leadingAnchor.constraint(equalTo: checkboxStackView.leadingAnchor, constant: 0).isActive = true
        copyBeforeUploadCheckbox.leadingAnchor.constraint(equalTo: checkboxStackView.leadingAnchor, constant: 0).isActive = true
        copyOnlyFromMonosnapCheckbox.leadingAnchor.constraint(equalTo: checkboxStackView.leadingAnchor, constant: 20).isActive = true
        closeMonosnapAfterUploadCheckbox.leadingAnchor.constraint(equalTo: checkboxStackView.leadingAnchor, constant: 20).isActive = true
        uploadCopiedFilesCheckbox.leadingAnchor.constraint(equalTo: checkboxStackView.leadingAnchor, constant: 0).isActive = true
        ...
        NSLayoutConstraint.activate([
            ...
            // Clipboard Format
            clipboardFormatLabel.topAnchor.constraint(equalTo: imageSettingsHeader.bottomAnchor, constant: 5),
            ...
            // JPG Quality
            jpgQualityLabel.topAnchor.constraint(equalTo: clipboardFormatLabel.bottomAnchor, constant: 5),
            ...
            hotkeySectionHeader.topAnchor.constraint(equalTo: jpgQualityLabel.bottomAnchor, constant: 5),
            hotkeySectionHeader.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),

            // Hotkey Description
            hotkeyDescriptionLabel.topAnchor.constraint(equalTo: hotkeySectionHeader.bottomAnchor, constant: 3),
            hotkeyDescriptionLabel.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),

            // Hotkey Recorder
            hotkeyRecorderView.topAnchor.constraint(equalTo: hotkeyDescriptionLabel.bottomAnchor, constant: 3),
            ...
            hotkeyRecorderView.heightAnchor.constraint(equalToConstant: 20),
            ...
            // Компактный блок чекбоксов (выровнен по левому краю как другие элементы)
            checkboxStackView.topAnchor.constraint(equalTo: hotkeyRecorderView.bottomAnchor, constant: 3),
            checkboxStackView.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 20),
            checkboxStackView.widthAnchor.constraint(equalToConstant: 440),
        ])
    }
    ...
    private func loadSettings() {
        ...
        // Load Close Monosnap After Upload Setting
        closeMonosnapAfterUploadCheckbox.state = defaults.bool(forKey: "closeMonosnapAfterUpload") ? .on : .off
    ...
    func saveSettings() {
        ...
        defaults.set(closeMonosnapAfterUploadCheckbox.state == .on, forKey: "closeMonosnapAfterUpload")
    ...
    @objc private func closeMonosnapAfterUploadChanged() {
        saveSettings()
        print("Закрывать окно Monosnap после загрузки: \(closeMonosnapAfterUploadCheckbox.state == .on)")
    }
