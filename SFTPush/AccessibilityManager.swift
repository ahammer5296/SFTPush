...
class AccessibilityManager {
    ...
    /// Симулирует нажатие комбинации клавиш Cmd+W для закрытия окна
    func simulateCloseWindow() {
        guard hasAccessibilityPermissions() else {
            print("Нет прав на управление другими приложениями")
            return
        }

        // Симулируем нажатие Cmd+W
        let keyDownEvent = CGEvent(keyboardEventSource: nil, virtualKey: CGKeyCode(kVK_ANSI_W), keyDown: true)
        keyDownEvent?.flags = CGEventFlags.maskCommand

        let keyUpEvent = CGEvent(keyboardEventSource: nil, virtualKey: CGKeyCode(kVK_ANSI_W), keyDown: false)
        keyUpEvent?.flags = CGEventFlags.maskCommand
        ...
        print("Симулировано нажатие Cmd+W")
    }
