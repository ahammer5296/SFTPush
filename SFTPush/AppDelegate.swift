...
class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate { // Изменено на UNUserNotificationCenterDelegate
        ...
        if copyBeforeUpload {
            ...
            if copyOnlyFromMonosnap {
                ...
                if AccessibilityManager.shared.isActiveApplicationMonosnap() {
                    ...
                    // Даем время на копирование (0.2 секунды), затем загружаем и закрываем окно
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                        self.uploadFromClipboardAndCloseMonosnap()
                    }
                } else {
    ...
    @objc func uploadFromClipboardAndCloseMonosnap() {
        // Сначала загружаем из буфера
        uploadFromClipboard()

        // Затем проверяем настройки и закрываем окно Monosnap если нужно
        let defaults = UserDefaults.standard
        let closeMonosnapAfterUpload = defaults.bool(forKey: "closeMonosnapAfterUpload")

        if closeMonosnapAfterUpload {
            // Проверяем права на управление другими приложениями
            if !AccessibilityManager.shared.hasAccessibilityPermissions() {
                print("Нет прав на управление другими приложениями для закрытия окна Monosnap")
                return
            }

            // Проверяем, является ли активное приложение Monosnap
            if AccessibilityManager.shared.isActiveApplicationMonosnap() {
                print("Закрываем окно Monosnap после загрузки")

                // Даем время на завершение загрузки и отправку уведомления
                DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                    AccessibilityManager.shared.simulateCloseWindow()
                }
            } else {
                print("Активное приложение не Monosnap, окно не закрываем")
            }
        }
    }
